...
def _add_missing_columns(conn):
    ...
    c = conn.cursor()
    # Add 'sector' and 'market_cap' to portfolio table if they don't exist
    ...
    # Add 'type' to expenses table if it doesn't exist
...
def fetch_stock_info(symbol):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Handle ETFs where 'sector' might be missing and 'fundFamily' or 'category' is available
...
def update_stock_data(symbol):
    ...
    try:
        ...
        # Fetch data up to tomorrow's date to ensure today's data is included
...
def get_benchmark_comparison_data(holdings_df, benchmark_choice):
    ...
    try:
        ...
        for _, row in holdings_df.iterrows():
            ...
            daily_units.loc[daily_units.index >= buy_date, row['symbol']] = row['quantity']
            daily_invested.loc[daily_invested.index >= buy_date, row['symbol']] = row['quantity'] * row['buy_price']
